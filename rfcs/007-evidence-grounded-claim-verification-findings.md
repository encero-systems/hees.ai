...
3. **Hees.ai owns the check manifest.** Hees.ai derives an ordered bounded list of visible-answer-unit and support-claim checks from one normalized candidate and one accepted RFC 003 memory context. The provider cannot add, remove, reorder, or rewrite a check.
...
5. **Hees.ai classifies deterministically.** Hees.ai validates the complete result, derives `Supported`, `Contradicted`, `Unsupported`, or `Uncertain` under package thresholds, applies package mappings, and selects one strongest non-authoritative action/reason pair.
...
This RFC defines that shared boundary. It preserves the useful signal from an external verifier while requiring package authority, Hees.ai-owned validation, RFC 004 finding composition, RFC 009 response governance, and RFC 001 Spectrum finality around it.
...
- Distinguish valid provider unavailability from malformed or incomplete results and fail closed in both cases through package-owned Hees.ai policy.
...
- Segmenting an opaque visible-output string inside this RFC. RFC 009 supplies normalized ordered visible units and owns their bounded visible-unit contract.
...
This RFC describes a proposed public contract. Hees.ai 0.0.1 does not implement provider-bound claim verification, candidate check manifests, semantic classifications, or verification-derived constraint findings.

An admitted package may declare one claim-verification policy and one or more exact provider bindings. The policy identifies the RFC 004 constraint through which verification participates in governance, sets separate thresholds for visible answer units and support claims, maps each Hees.ai-derived classification to one package-owned constraint action/reason pair, and defines one fail-closed pair for valid provider unavailability.

RFC 009 supplies one normalized candidate. The candidate has a logical `proposal_id`, Hees.ai-owned `attempt_index`, ordered visible answer units, and ordered atomic support claims derived from those visible units. Each support claim cites exactly one governed-memory atom. Hees.ai constructs the admitted closed verification-subject model and derives its `candidate_structural_identity` through RFC 011. That identity binds the exact validated candidate content and order for verification; it is not the logical proposal identifier and must not appear as a substitute for `proposal_id`.

Hees.ai then creates checks in one exact order. Every visible answer unit comes first in visible order, and support claims follow in support order. Each visible-answer-unit check carries the complete ordered memory identifiers materialized by one accepted RFC 003 context. Each support-claim check carries exactly its one cited memory identifier. Target text is copied from the normalized candidate, never supplied independently by the provider.
...
This RFC depends normatively on RFC 003 accepted memory contexts, RFC 004 finding and adjudication semantics, RFC 005 package admission, RFC 011 structural identity, and the RFC 009 normalized candidate contract. RFC 006 remains the exclusive owner of receipt encoding and exports no detailed verifier data under receipt contract `0.1`.

RFC 005 artifact contract `0.1` incorporates `claim_verification` in its closed profile topology. It owns the member kind, order, record count, resource envelope, cross-member classification, retained indexes, and atomic package identity; this RFC owns the exact payload and verifier-finding semantics. Adding the member requires a new artifact contract and is not an implementation-local extension.

RFC 009 owns visible-unit identity, support-claim identity, the exact derivation of atomic support-claim text from a visible unit, attempt identity, and candidate normalization. RFC 007 must consume those settled values without inferring sentence boundaries from an opaque string or accepting provider-authored support prose as a substitute.
...
| --- | --- | --- |
| `provider_bindings` | array of provider-binding objects | Required, non-empty, ordered, unique by binding identifier and complete binding tuple, and bounded by the RFC 005 profile resource envelope. |
...
The package declaration establishes exact expected identity, not external authenticity or proof of execution. Hees.ai may compare a binding identifier and materialize its admitted fields, but it must not claim that it independently hashed the provider, model, configuration, tokenizer, executable, or runtime environment.
...
| `allowed_binding_ids` | array of strings | Required non-empty ordered subset of admitted binding identifiers, bounded by the RFC 005 profile resource envelope, and duplicate-free. |
...
Every threshold must be an exact integer in inclusive `5001..10000`. Fractional, exponent-form, negative, zero, absent, or out-of-range values are invalid. Requiring a strict majority ensures that no two relation scores can meet their threshold when the score triple sums to `10000`.
...
`supported_finding.action` must be `continue`. `contradicted_finding.action` must be `reject` or `escalate`. `unsupported_finding.action` and `uncertain_finding.action` may be `revise`, `reject`, or `escalate`, but must not be `continue` or `clarify`. `unavailable_finding.action` must be `reject` or `escalate` and applies identically to every recognized unavailable reason so provider-selected reason spelling cannot weaken policy.
...
Hees.ai must construct a closed verification-subject projection containing exactly these fields and closed nested models:

```incan
pub model VerificationVisibleAnswerUnit:
    pub unit_id: str
    pub text: str
    pub requirement_ids: list[str]


pub model VerificationSupportMapping:
    pub support_claim_id: str
    pub unit_id: str
    pub evidence_ids: list[str]
    pub memory_ids: list[str]


pub model VerificationSubject:
    pub attempt_index: int
    pub candidate_id: str
    pub proposal_id: str
    pub visible_answer_units: list[VerificationVisibleAnswerUnit]
    pub support_mappings: list[VerificationSupportMapping]
```

The values above are illustrative, but the field names, nesting, and array order are exact. The projection copies the RFC 009 proposal, attempt, selected candidate, visible-unit, requirement, support-mapping, evidence, and memory values without normalization or omission. Derived support-claim text is not duplicated because it is exactly the referenced visible-unit text. Hees.ai must construct this validated model and derive `candidate_structural_identity` through the registered RFC 011 `hees.verification_candidate` identity domain. It is a typed `StructuralIdentity`, not an arbitrary `sha256:` text value.

`proposal_id` and `candidate_structural_identity` have different meanings. The proposal identifier is the RFC 009 logical identity and must never be derived from visible output. The candidate structural identity binds exact normalized verification content and order for request/result replay protection. It must not replace proposal identity, become a receipt identifier, or be presented as proof of semantic correctness.

A finding produced for one attempt or candidate structural identity must not be accepted for another attempt or identity, even when all target identifiers remain the same.
...
Hees.ai must create one check for every normalized visible answer unit, followed by one check for every normalized atomic support claim. Visible-answer-unit checks preserve visible-unit order. Support-claim checks preserve support-claim order. The total ordered check manifest is part of request identity.

Each check must contain exactly:

- `check_id`, a Hees.ai-derived canonical identifier;
- `target_kind`;
...
- the candidate structural identity inherited from the containing request rather than repeated inside the check.

Hees.ai must derive check identifiers as `visible_answer_unit_check_<zero-based ordinal>` and `support_claim_check_<zero-based ordinal>` using canonical decimal ordinals without leading zeroes except ordinal zero. The identifiers are stable only within one candidate structural identity and check manifest; they are not global content identities.
...
Hees.ai must not accept an opaque visible string and infer sentence boundaries under this contract. It must not accept a caller-selected subset of visible units, an independently rewritten claim, a claim with zero or multiple memory references, or a premise list supplied by the provider.
...
- `package_identity`, copied as the trusted RFC 005 `AdmittedPackageIdentity`;
...
- `candidate_structural_identity`, computed by Hees.ai from the exact projection above;
...
After constructing and validating the complete request, Hees.ai must derive `verification_request_identity` through the registered RFC 011 `hees.verification_request` identity domain. The identity covers every request field, including the complete package identity, policy, binding, proposal, attempt, candidate, evaluation time, admitted-memory context, and check-manifest values. It is result-linkage identity only: it is not a receipt, authenticity proof, globally unique operation identifier, or substitute for any covered typed identity.
...
- `verification_request_identity`, exactly the Hees.ai-computed request structural identity;
- `result_id`, one canonical bounded caller result identifier;
- `candidate_structural_identity`, exactly the request candidate structural identity;
...
The result and each observation must not contain target text, premise text, memory identifiers, evidence identifiers, support references, package or policy actions, terminal decisions, labels selected by the provider, free-form rationale, arbitrary diagnostic strings, hidden reasoning, provider paths, model names, runtime environment, or extension maps. Hees.ai resolves the admitted binding from `binding_id`; it must not trust provider-returned fingerprint fields. It must compare the result's verification-request identity with its independently computed trusted identity before using any observation.

### Classification

For each observation in a valid complete result, Hees.ai must select the target rule by the request check's target kind and classify the score triple in this exact order:
...
The RFC 005 Package profile resource envelope must declare the closed exact ceilings for `claim_verification` member bytes, parser nesting and fields, provider bindings, allowed bindings, identifiers, provider/model/configuration identity values, normalized candidate text, visible units, support claims, total checks, premise identifiers, request bytes, result bytes, observations, retained binding and policy indexes, detailed record retention, and operator-trace projection. It must also declare the accounting rule that composes these values with RFC 003 selected memory and RFC 009 response state.

This RFC adds two semantic rules that are not adjustable resource limits: one support claim has exactly one cited governed-memory atom, and every complete provider result contains exactly one observation for every Hees.ai-created check. It also retains the fixed-point score domain: every relation score is an exact integer in `0..10000`, and each support, contradiction, and unresolved triple sums to exactly `10000` through checked arithmetic.

Hees.ai must enforce raw byte ceilings before parsing and collection ceilings before proportional allocation or retention. Checked sums must use the RFC 005 exact-integer domain and must not wrap, saturate, clamp, or depend on native integer width. RFC 006 requires no new receipt-size allowance because verifier details are not exported. Conformance evidence must prove that the profile envelope, package admission, verification request, provider result, and RFC 009 candidate are mutually consistent. Physical RSS, latency, energy, thermal behavior, and model coexistence remain recorded deployment measurements rather than public verification reasons.

### Determinism and privacy

Given identical RFC 005-admitted package data, identical RFC 009 normalized candidate data, the same accepted RFC 003 context, and an identical typed provider result, conforming implementations must produce the same candidate structural identity, verification-request identity, check manifest, record variant, public reason, per-check classifications, primary check, selected package mapping, and derived RFC 004 finding.
...
RFC 005 owns the common member wrapper, exact binary member bytes, member digest and length, final member order, sequential admission, record count, `AdmittedPackageIdentity`, compact retained indexes, profile resource envelopes, and atomic package completion. This RFC owns only the closed `claim_verification` payload and its cross-reference semantics.
...
The RFC 004 constraint receipt may identify the linked constraint as evaluated or substituted under RFC 006's existing structural projection. Complete verifier findings, scores, provider bindings, candidate structural identities, and premise identifiers remain outside the receipt. Successful verification is not proof that admitted identifiers semantically establish real-world truth.
...
Authority-negative API fixtures must prove that a provider cannot change candidate text or references, add memory, select an action, construct an RFC 004 result, emit an authoritative receipt, or turn successful public receipt verification into in-process Hees.ai authority. Receipt fixtures must reject every attempt to include candidate structural identity, provider/model/configuration fingerprint, target text, scores, classifications, or detailed verifier findings in contract `0.1` bodies.
...
Rejected because a provider-selected label or terminal action would make provider configuration an implicit decision authority. The provider returns only bounded relation features; Hees.ai derives classifications and RFC 004 owns the action.
...
### Add provider bindings outside RFC 005 contract `0.1`

Rejected because RFC 005 uses a closed member topology. A later member requires a new artifact contract rather than an ignored extension.
...
The verifier's guaranteed granularity is the RFC 009 bounded visible unit. This contract does not claim that a check corresponds to one grammatical sentence or atomic proposition. The same exact visible-unit model preserves multilingual shape without an implementation-defined segmentation step.
...
- **Candidate integration:** Exact RFC 009 proposal, attempt, visible-unit, derived support-claim, and candidate structural-identity binding without taking ownership of candidate lifecycle.
...
- `candidate_structural_identity` is an RFC 011 structural identity for the exact normalized verification subject and is never the logical proposal identifier or a receipt identifier.
- `verification_request_identity` is an RFC 011 structural identity that binds the complete trusted request, including memory premises and check manifest, and must be echoed by the provider result before observations can be used.
...
- RFC 005 artifact contract `0.1` incorporates this member in its closed profile topology; a later member addition requires a new exact artifact contract.
...
- Hees.ai derives `Contradicted`, `Supported`, `Unsupported`, or `Uncertain` and maps each through package policy.
...
- Detailed targets, scores, classifications, candidate structural identities, and provider bindings remain outside RFC 006 receipt contract `0.1`.
- The RFC 005 profile resource envelope owns member, request, result, and retained-state ceilings. Its conformance evidence must cover multilingual shape and every supported Package language/capability profile.
