...
- `artifact_digest` is not a caller-chosen label: `digest_continuity_package` recomputes it from every other package field (goals and schedules included) and `validate_continuity_package` rejects (`artifact_digest_mismatch`) if the supplied value doesn't match, so a package that was mutated after its digest was stamped fails closed rather than silently carrying a stale identity;
...
`SessionContinuityState` is the caller-reconstructed result of the last admitted decision: session and package identity, current goal/schedule/phase, when the goal and the current phase started, the last admitted clock value and event index, whether the goal is closed, and a `state_witness` digest. Hees.ai never derives this from storage; the caller supplies it as part of the proposal and Hees.ai only checks it for internal and cross-proposal consistency. `state_witness` is a self-consistency digest that `evaluate_continuity` stamps onto every state it returns and re-verifies (`prior_state_witness_invalid`) against the rest of the object's own fields before trusting a supplied `prior_state` — this catches a state hand-assembled or corrupted outside `evaluate_continuity`'s own output, but it is not a cryptographic signature: the digest formula is public source, so it cannot by itself stop a caller willing to reproduce it. A real signed handle needs a keyed MAC, which Incan's standard library does not yet provide (tracked as [incan#1075](https://github.com/encero-systems/incan/issues/1075)); see [Open questions](#open-questions).
...
- **Start**: requires `prior_state` to be absent (`start_requires_empty_state`), `event_index == 0` (`start_event_index_invalid`), and the proposed phase to be the goal's declared `entry_phase_id` (`start_phase_not_declared`). Admits with reason `declared_goal_started`; both `started_at_seconds` and `phase_started_at_seconds` are set to the proposal's clock value, and `evaluate_continuity` stamps a fresh `state_witness` on the resulting state.
- **Continue**, **Transition**, **Close** all require `prior_state` to be present (`prior_state_required`), and then:
    ...
    - the prior state's `state_witness` must match a fresh recomputation over its own other fields (`prior_state_witness_invalid`) — this rejects a `prior_state` that was hand-assembled or corrupted rather than taken verbatim from an earlier `evaluate_continuity` result;
    ...
    - the elapsed time in the prior phase (`current_clock_seconds - prior.phase_started_at_seconds`) must not exceed that phase's declared `maximum_seconds` (`phase_maximum_exceeded`) — checked for all three operations, not just `Transition`, so a session cannot sit in one phase past its declared maximum by repeatedly proposing `Continue`.
...
`invalid_continuity_package`, `package_identity_mismatch`, `unknown_goal`, `unknown_schedule`, `schedule_goal_mismatch`, `clock_source_not_allowed`, `unknown_phase`, `action_not_allowed_for_goal`, `action_not_allowed_for_phase`, `invalid_event_coordinates`, `start_requires_empty_state`, `start_event_index_invalid`, `start_phase_not_declared`, `prior_state_required`, `prior_state_identity_mismatch`, `prior_state_witness_invalid`, `session_already_closed`, `event_index_not_monotonic`, `clock_regression`, `prior_phase_unknown`, `phase_maximum_exceeded`, `continue_phase_mismatch`, `phase_transition_not_declared`, `phase_minimum_not_met`, `goal_close_not_declared`, `goal_budget_not_reached`, `unknown_continuity_operation` (rejections); and `declared_goal_started`, `continue_current_phase`, `declared_phase_transition`, `declared_goal_closed` (admissions).

`invalid_continuity_package` is a coarse wrapper: every structural package-graph failure (empty declarations, duplicate identifiers, a non-topological phase graph, and now a stale `artifact_digest`) collapses into it at the `evaluate_continuity` boundary. A caller who wants the specific cause calls `validate_continuity_package` directly and reads its own `reason` field, which is not coarsened (e.g. `artifact_digest_mismatch`, `continuity_declarations_empty`, `phase_graph_not_topological`).
...
A caller must implement its own persistence adapter to get any value from this contract; the RFC defines no storage shape, so independent implementations may reasonably diverge on log format even while agreeing on admission semantics. The phase-graph-must-point-forward rule (used here to guarantee acyclicity by construction) forbids expressing a schedule where an earlier-declared phase is revisited from a later one, even where a package author might reasonably want a bounded loop (e.g., "review, then optionally repeat one earlier phase"); the current contract has no support for that shape. The admission-reason vocabulary above is not yet closed or namespaced the way RFC 003/006 close theirs, so it may need a breaking revision once formalized. `state_witness` is real but limited: an unkeyed self-consistency digest cannot stop a caller who is willing to read this public source and reproduce the formula, and it cannot detect a caller who calls `evaluate_continuity` honestly but supplies a fabricated `current_clock_seconds` at every step, since the caller-supplied clock is trusted by design (see [Alternatives considered](#alternatives-considered)). Closing that gap needs a keyed MAC where the key lives only in the trusted caller's custody, which is not yet available in Incan's standard library.

## Layers affected

- **Public contract:** New `ContinuityOperationKind`, `ContinuityTerminal`, `GovernedGoal`, `GovernedPhase`, `GovernedSchedule`, `ContinuityPackage`, `SessionContinuityState` (now carrying `state_witness`), `ContinuityProposal`, `ContinuityDecision`, and `ContinuityValidation` types; new `validate_continuity_package` and `evaluate_continuity` public functions, plus `digest_continuity_package` and `digest_continuity_state_witness` for callers to stamp a package's real digest and a state's real witness before use.
...
- A phase's declared `minimum_seconds` gates `Transition` away from it; the schedule's `total_target_seconds` gates `Close`; a phase's declared `maximum_seconds` gates all three operations uniformly, so a session cannot remain in one phase past its declared maximum by repeatedly proposing `Continue`.
- `SessionContinuityState` carries a `state_witness` self-consistency digest that `evaluate_continuity` stamps and re-verifies, so a `prior_state` must be taken verbatim from an earlier result rather than hand-assembled. This is honestly scoped: it is an unkeyed digest over public source, so it catches accidental corruption and casual fabrication, not a caller willing to reproduce the formula — a real signed handle needs a keyed MAC, which is tracked upstream as [incan#1075](https://github.com/encero-systems/incan/issues/1075) rather than hand-rolled here.
...
- `state_witness` should become a keyed MAC once Incan's standard library supports one ([incan#1075](https://github.com/encero-systems/incan/issues/1075)), so `evaluate_continuity` can actually authenticate a `prior_state` that crossed an untrusted boundary (e.g. a network client), not just detect accidental corruption or naive hand-assembly. Should the key be a required parameter on `evaluate_continuity` from the start of that work, or should the unkeyed digest remain a supported mode for callers with no untrusted boundary to defend?
