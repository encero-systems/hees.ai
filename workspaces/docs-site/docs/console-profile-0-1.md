...
The earlier immutable-source GitHub matrix built, extracted, and smoke-tested exact-head candidates on macOS ARM64, macOS x86-64, and Linux x86-64 with the then-pinned `0.4.0` compiler. Those results are historical candidate evidence for their source commit. The current head has zero release artifacts. It requires the commit-pinned Incan `0.5.0-dev.22` compiler and must produce a fresh three-platform matrix before any current support claim. A platform becomes judge-facing only when its exact audited artifact is published with its checksum, provenance, dependency notices, and test instructions. Windows and Linux ARM64 remain unsupported.
