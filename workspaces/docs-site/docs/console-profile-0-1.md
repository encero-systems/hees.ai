...
The earlier immutable-source GitHub matrix built, extracted, and smoke-tested exact-head candidates on macOS ARM64, macOS x86-64, and Linux x86-64 with the then-pinned `0.4.0` compiler. Those results are historical candidate evidence for their source commit. The immutable `hees-console-v0.1.0` tag used its commit-pinned Incan `0.5.0-dev.23` compiler; the current source branch pins `0.5.0-dev.32` for subsequent development and release candidates. Support remains established only for platforms whose exact audited artifacts are published with checksums, provenance, dependency notices, and test instructions. Windows and Linux ARM64 remain unsupported unless a later tagged release adds equivalent evidence.
