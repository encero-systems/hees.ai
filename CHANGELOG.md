...
- Session-local Profile Studio state for staging evidence and memory, validating candidates through the real Hees acceptance probe, blocking unsupported activation, and restoring the shipped profile.
- An original fictional lesson-support profile with one admitted interaction and four adversarial rejection scenarios.
- Inspectable Hees findings, bounded Spectrum evaluation, selected memory, Content DNA, receipts, and a separately labelled non-authoritative trace.
- Provider-neutral Training by Committee target derivation, bounded observations, coverage validation, and Hees-owned classification.
- Integrity-checked offline replay that stores inputs and metadata but never stores a reusable Hees decision.
...
- Native packaging and release-validation workflows for unsigned macOS and Linux candidates, including checksums, provenance, dependency notices, leakage scans, and extracted-archive smoke tests. No current-head release artifacts have been published.
...
No `0.1.0` compatibility promise or release tag exists yet.
